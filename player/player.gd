...
func set_display_mode(mode: bool):
	if mode:
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
		$SpringArmPivot.process_mode = Node.PROCESS_MODE_DISABLED
		$SpringArmPivot/Camera3D/Crosshair.visible = false
		disable_controller = true
	else:
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
		$SpringArmPivot.process_mode = Node.PROCESS_MODE_INHERIT
		$SpringArmPivot/Camera3D/Crosshair.visible = true

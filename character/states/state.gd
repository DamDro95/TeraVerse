...
func check_combos(input : InputPackage):
	for combo : Combo in combos:
		if model.combo.is_triggered(input) and model.stats.can_be_paid(model.container.moves[combo.triggered_move]):
			has_queued_move = true
			queued_move = combo.triggered_move


func best_input_that_can_be_paid(input : InputPackage) -> String:
	input.actions.sort_custom(model.states.moves_priority_sort)
	for action in input.actions:
		if model.stats.can_be_paid(model.states.moves[action]):
			if model.states.moves[action] == self:
				return "okay"
			else:
				return action
...
func transitions_to_queued() -> bool:
	return state_data.get_transitions_to_queued(backend_animation, get_progress())

func accepts_queueing() -> bool:
	return state_data.get_accepts_queueing(backend_animation, get_progress())

func tracks_input_vector() -> bool:
	return state_data.tracks_input_vector(backend_animation, get_progress())

func time_til_unlocking() -> float:
	if tracks_input_vector():
		return 0
	return state_data.time_til_next_controllable_frame(backend_animation, get_progress())

func is_vulnerable() -> bool:
	return state_data.get_vulnerable(backend_animation, get_progress())

func is_interruptable() -> bool:
	return state_data.get_interruptable(backend_animation, get_progress())

func is_parryable() -> bool:
	return state_data.get_parryable(backend_animation, get_progress())

func get_root_position_delta(delta_time : float) -> Vector3:
	return state_data.get_root_delta_pos(backend_animation, get_progress(), delta_time)

func right_weapon_hurts() -> bool:
	return state_data.get_right_weapon_hurts(backend_animation, get_progress())
...
func try_force_move(new_forced_move : String):
	...
	elif model.container.moves[new_forced_move].priority >= model.container.moves[forced_move].priority:
		forced_move = new_forced_move
